...
class FracGraph:
    def __init__(self, connection_radius, error = 0.1, start_buffer = 0.5, test_mode = False, simulation_temp = 300, units = "real", minimize = False, load_margin = 0):
        sys.stdout.flush()
        sys.stderr.flush()
        time.sleep(1)
        comm.Barrier()
        ...
        self.__head.set_tip(tuple(self.__trunc(start_pos, 3)))
        self.__tail.set_tip(tuple(self.__trunc((x_side/2 + box[0][0], Data.old_bounds[1] + start_buffer), 3)))
        ...
        self.__step_energies[self.__head.get_id()] = [0]
    ...
    def build_test(self):
        ...
        node1 = self.attach(coords = (20, 10))
        node2 = self.attach(coords = (10, 30))
        node3 = self.attach(coords = (20, 30))
        node4 = self.attach(coords = (30, 30))
        ...
        self.__tail.attach(node2)
        self.__tail.attach(node3)
        #self.__tail.attach(node4)
    ...
    @Helper.linear_func
    def recalculate_path(self, path):
    ...
    def calculate(self):
        ...
        node_loads = {node_id : float("inf") for node_id in range(self.__node_ctr)}
        while node_queue:
            #Helper.mpi_print(sorted(node_queue, key=lambda x: x[0]))
            node_load, node = heapq.heappop(node_queue)
            if node.is_tail():
                if node_load < self.__min_complete_load:
                    self.__min_complete_load = node_load
                    Helper.mpi_print("New minimum load of a complete pathway:", self.__min_complete_load, verbosity = 2)
                self.__tail_versions.append({"load": node_load, "wall_list" : node.wall_list.copy(), "area" : node.get_surface_area()})
                continue

            if node_load <= node_loads[node.get_id()] + self.__load_margin and node_load <= self.__min_complete_load + self.__load_margin:
                if node_load < node_loads[node.get_id()]:
                    node_loads[node.get_id()] = node_load
                Helper.mpi_print("On node", node, "with load:", node_load, "and parent:", node.get_parent(), verbosity = 1)
                ...
                for neigh in neighbors:
                    if neigh.get_pos()[1] > current_pos[1]:
                        neigh_load = neigh.activate(parent = node)
                        Helper.mpi_print("Explored neighbor", neigh.get_id(), " with load", neigh_load, verbosity = 4)
                        if neigh_load > node_load:
                            max_path_load = neigh_load
                        else:
                            max_path_load = node_load


                        selected = False
                        if neigh.is_tail() and (neigh.get_id() in self.__paths) and max_path_load <= self.__min_complete_load + self.__load_margin:
                            selected = True
                            Helper.mpi_print("Found new tail with parent:", node, "and path load:", max_path_load, verbosity = 2)

                        elif max_path_load < node_loads[neigh.get_id()] and max_path_load <= self.__min_complete_load + self.__load_margin:
                            selected = True
                            Helper.mpi_print(f"Node {neigh.get_id()} was accepted with path load of {max_path_load}", verbosity = 2)


                        if selected:
                            heapq.heappush(node_queue, (max_path_load, neigh))

                            #Adding to paths for coloring
                            neigh_id = neigh.get_id()
                            if neigh_id in self.__paths:
                                self.__paths[neigh_id].add(node.get_id())
                            else:
                                self.__paths[neigh_id] = {node.get_id()}

                            if neigh_id in self.__step_energies:
                                self.__step_energies[neigh_id].append(neigh_load)
                            else:
                                self.__step_energies[neigh_id] = [neigh_load]
                        else:
                            Helper.mpi_print("Ignored neigh:", neigh, verbosity = 4)
        ...
        for tail_inst in self.__tail_versions:
            Helper.mpi_print("---------------------", verbosity = 1)
            self.__head.wall_list = tail_inst["wall_list"]
            energy_difference = self.__head.reset_lowest(reset_head = True) - self.__head.get_pe()
            crack_surface_area = tail_inst["area"]
            energy_release_rate = 0.69*energy_difference/crack_surface_area
            experimental_energy_release_rate = 0.69*energy_difference/((Data.old_bounds[1] - Data.old_bounds[0])*(self.__box[1][2] - self.__box[0][2]))
            Helper.mpi_print("Energy diff:", energy_difference, verbosity = 1)
            Helper.mpi_print("Surface Area", crack_surface_area, verbosity = 1)
            Helper.mpi_print("G:", energy_release_rate, verbosity = 1)
            Helper.mpi_print("ExG:", experimental_energy_release_rate, verbosity = 1)
    ...
    def __rec_path_search(self, node_id):
        if self.__id_node_map[node_id].is_head():
            return [[(*self.__id_node_map[node_id].get_pos(), float(self.__step_energies[node_id][0]))]]

        local_paths = list()
        for i, parent in enumerate(self.__paths[node_id]):
            for par_path in self.__rec_path_search(parent):
                local_paths.append([(*self.__id_node_map[node_id].get_pos(), float(self.__step_energies[node_id][i]))] + par_path)
    ...
    def get_paths(self):
        ...
        non_leafs = set().union(*self.__paths.values())
        for node_id in self.__paths.keys():
            if node_id not in non_leafs:
    ...
    def attach(self, coords):
        ...
        if disc_coords in self.__node_hash:
            ...
            Helper.mpi_print("Created a new node", self.__node_ctr, "at pos:", disc_coords, verbosity = 1)
            self.__node_ctr += 1
...
class Node:
    ...
    def set_parent(self, node):
        if node:
            ...
            Helper.mpi_print(f"AFTER RESET ENG({node.__id}): {self.__parent_pe}", verbosity = 3)
    ...
    def reset_lowest(self, reset_head = False):
        if not self.is_head() or reset_head:
            Helper.mpi_print("Resetting lowest:", self.__id, verbosity = 4)
            self.__start_lammps()
            ...
            Helper.mpi_print(f"AFTER RESET ENG({self.__id}): {after_reset_pe}", verbosity = 3)
            return after_reset_pe



    def __start_lammps(self):
        Helper.mpi_print("Starting lammps for node", self.__id, verbosity = 4)
        if self.__test_mode:
    ...
    def activate(self, parent = None):
        ...
        Helper.mpi_print(f"BEFORE MIN ENG({self.__id}): {self.prerelax_pe}", verbosity = 3)
        if self.__minimize and not self.__is_tail:
        #if self.__minimize:
        #if self.__minimize and not self.__is_tail and not self.__is_head:
            self.__lmp.command("minimize 1.0e-8 1.0e-8 100000 10000000")
            """
            if self.__is_head:
                self.__lmp.command("minimize 1.0e-8 1.0e-8 100000 10000000")
            else:
                self.__lmp.command(f"fix min all npt temp {self.__temp} {self.__temp} $(100*dt) iso 0 0 $(1000*dt)")
                self.__lmp.command("run 1000")
                self.__lmp.command("unfix min")
            """

        self.__active = True
        self.__pe = self.__lmp.get_thermo("pe")
        Helper.mpi_print(f"AFTER MIN ENG({self.__id}): {self.__pe}", verbosity = 3)
        ...
        if self.__is_head:
            self.__load = 0
            Helper.mpi_print(f"Node {self.__id}, activated (HEAD)", verbosity = 1)
        else:
            G = 0.69*(self.prerelax_pe - self.__parent_pe)/self.__surface_area
            a = self.__cut_length
            if G < 0:
                G = 0
            self.__load = np.sqrt(G/(a*(np.cos(np.pi/2 - self.__theta))**2))

            #self.__lmp.command(f"region slab_{self.current_wall_count} delete")
            #self.__lmp.command(f"unfix wall_{self.current_wall_count}")

            Helper.mpi_print(f"Node {self.__id}, activated at x = {round(self.__tip[0], 3)}, y = {round(self. __tip[1], 3)}, Parent: {self.__parent.__id}, energy_release: {round(G, 3)}, load: {round(self.__load, 3)}", verbosity = 1)
    ...
    def add_wall(self, wall, wall_num = None):
        ...
        Helper.mpi_print(f"Adding wall {wall_num}", verbosity = 4)
