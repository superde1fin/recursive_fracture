...
def quazi_static(lmp, dr_frac = 0.01, dtheta = 1):
    start_coords = (lmp.eval("lx")/2 + lmp.system.xhi, 0)
    new_lmp = QSR(lmp = lmp, coords = start_coords, dr = dr_frac*max(lmp.eval("lx"), lmp.eval("ly"), lmp.eval("lz")), dtheta = dtheta, norm = (1, 0))

    return (lmp.eval("pe") - new_lmp.eval("pe"))/new_lmp.variables["surface_area"].value



def QSR(lmp, coords, dr, dtheta, norm, theta):
    lmp = copy_lmp(lmp)
    return 

    if coords[0] > lmp.eval("lx") + lmp.eval("xlo") or coords[0] < lmp.eval("xlo") or coords[1] > lmp.eval("ly") + lmp.eval("ylo") or coords[1] < lmp.eval("ylo"):
        return lmp

    """
    if "surface_area" not in lmp.variables.keys():
        lmp.variable("surface_area equals 0.0")
    else:
        lmp.variable("surface_area equals ${surface_area}")
    """
    ...
    res_lmp = lmp
    for theta in np.linspace(0, np.pi, int(180/dtheta)):
        os.system(f"mkdir {theta} && cd {theta}")
        tmp_lmp =  QSR(lmp, coords = (coords[0] + dr*np.cos(theta), coords[1] + dr*np.sin(theta)), dr = dr, dtheta = dtheta, norm = (np.cos(theta - np.pi/2), np.sin(theta - np.pi/2)), theta)
        os.system("cd ..")
